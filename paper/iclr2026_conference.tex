...
\begin{itemize}
  \item \textbf{The acceptance layer, made explicit.} We give a taxonomy of
  seven families of acceptance rules for multi-token proposals and implement them
  behind a single code path, so that two configurations differ only in the
  acceptance predicate. Alongside it we give a measurement protocol that
  separates throughput from faithfulness, including a test of whether the
  generated text is distributionally distinguishable from the base model's
  (\S\ref{sec:acceptance}).
