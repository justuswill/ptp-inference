...
% Main point / insight
Together, our improvements yield up to 10 tokens per call across a wide range of applications on several pretrained models, a 200\% increase over recent methods, unlocking the full high-throughput potential of inference.
\end{abstract}
...
\begin{itemize}
    ...
    \item design choices have been
    \item focused on exactly replicating teacher
    \item: now a trade-off (example p-threshold)
...
A multi-token decoder is fixed by three choices: which proposals count as
acceptable, what shape the proposals take, and what runs concurrently with
verification. Existing methods each occupy one point in that space, usually
without naming the space. Table~\ref{tab:baselines} places the Spec-Bench
baselines; Tables~\ref{tab:verification}--\ref{tab:parallel} enumerate the
options along each axis. Our own system is one further point, but the
contribution we care about is the space itself: once the three axes are
separated, most published designs are recognizable as special cases, and the
choices nobody has tried become visible.
...
\textbf{Parallelization.} Lookahead is the fused-pass row; Partial Quadratic
Decoding is the budgeted-branch row. \S\ref{sec:pqd} shows that the branch
...
proposal mechanism and Partial Quadratic Decoding we build on; the connection
